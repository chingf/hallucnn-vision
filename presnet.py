...
from   torch.nn import Sequential, Conv2d, MaxPool2d, Linear, ConvTranspose2d, Upsample
...
class PResNet18V3NSeparateHP(PNetSeparateHP):
    """
    3x3 conv transpose kernels and upsample with Gradient Scaling
    """
    def __init__(
        self, resnet, build_graph=False, random_init=True,
        ff_multiplier: float=0.33, fb_multiplier: float=0.33, er_multiplier: float=0.01,
        register_backbone_hooks=False
        ):
        ...
        pmodule = Sequential(Upsample(scale_factor=(2,2), mode='bilinear'),ConvTranspose2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1))
        ...
        if register_backbone_hooks:
            ...
            self.backbone.conv1.register_forward_hook(track_hook1)
            self.backbone.layer1[1].conv2.register_forward_hook(track_hook2)
            self.backbone.layer2[1].conv2.register_forward_hook(track_hook3)
            self.backbone.layer3[1].conv2.register_forward_hook(track_hook4)
            self.backbone.layer4[1].conv2.register_forward_hook(track_hook5)
